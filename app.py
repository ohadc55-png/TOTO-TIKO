...
def calculate_parallel_status(raw_data, initial_stake):
    ...
    for row in raw_data:
        try:
            ...
            cycle_investments[comp] += current_stake
...
if not filtered_df.empty:
    ...
    fig = px.area(filtered_df, x=filtered_df.index, y='Cumulative Profit', title=f"{selected_comp} Growth")
    fig.update_traces(line_color='#2d6a4f', fillcolor='rgba(45, 106, 79, 0.2)')
    st.plotly_chart(fig, use_container_width=True)

# --- DANGER ZONE (The Reset Button) ---
st.write("---")
with st.expander("⚠️ Danger Zone"):
    st.warning("This action will permanently delete ALL data from the database.")
    if st.button("Delete Database History"):
        if worksheet:
            # Resizes the sheet to 1 row (keeping only the headers)

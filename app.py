...
st.markdown(f"""
    ...
    /* --- 2 CHANGES: BANNERS PROPORTIONS & MOBILE FIX --- */
    .comp-banner-box {{
        border-radius: 15px;
    ...
    @media only screen and (max-width: 768px) {{
        ...
        .comp-banner-box {{ padding: 10px !important; }}
...
def calculate_logic(raw_data, br_base, af_base):
    ...
    for idx, row in enumerate(raw_data):
        try:
            ...
            if res == "Pending":
                processed.append({"Row": idx + 2, "Date": row.get('Date', ''), "Comp": comp, "Match": f"{row.get('Home Team','')} vs {row.get('Away Team','')}", "Odds": odds, "Expense": 0.0, "Income": 0.0, "Net Profit": 0.0, "Status": "⏳ Pending", "ROI": "N/
...
def get_data_from_sheets():
    try:
        ...
        competitions_sheet = sh.get_worksheet(1)  # NEW: Competitions sheet
...
def add_competition(competitions_sheet, name, initial_bet, goal, logo_url, color1, color2, text_color):
    """Add new competition to sheet."""
    if competitions_sheet is None:
        return False
    try:
        from datetime import datetime
        today = datetime.now().strftime("%Y-%m-%d")
        competitions_sheet.append_row([name, "Active", initial_bet, goal, logo_url, color1, color2, text_color, today, "", 0])
        get_data_from_sheets.clear()
        return True
    except Exception as e:
        st.error(f"Failed to add competition: {e}")
        return False

def get_active_competitions(competitions_data):
    """Get list of active competition names."""
